...
export class PrepolyLsp {
  ...
  private async run(text: string, requests: object[]): Promise<JsonRpcMessage[]> {
    const stdin = concat([
      frame({
        jsonrpc: "2.0",
        id: 0,
      ...
      frame({
        ...
        params: { textDocument: { uri: DOC_URI, languageId: "prepoly", version: 1, text } },
      }),
      ...requests.map(frame),
    ]);
    ...
    try {
      ...
      wasi.start(inst as unknown as { exports: { _start: () => unknown; memory: WebAssembly.Memory } });
    } catch (err) {
  ...
  private static reply(messages: JsonRpcMessage[], id: number): JsonRpcMessage | undefined {
  ...
  async diagnostics(text: string): Promise<Diagnostic[]> {
    ...
    const report = PrepolyLsp.reply(messages, 1)?.result as { items?: Diagnostic[] } | null;
    return report?.items ?? [];
  }
  ...
  async semanticTokens(text: string): Promise<number[]> {
    ...
    return ((PrepolyLsp.reply(messages, 1)?.result as { data?: number[] } | null)?.data) ?? [];
  }
}
