...
`.pp` extension; one file is one module (see [Modules](modules.md)).
...
(see [Type system](types.md#literals)). A literal too large for `int64` is a
lexing error.
...
- Postfix `!` is [error propagation](types.md#result). There is **no** postfix
...
- There is no general range operator. The bracket form `[lo..hi]` is an
  *expression* producing the half-open integer range `lo .. hi-1` as an array:
  `[1..5]` is `[1, 2, 3, 4]`.
...
- **Array/tuple literal** `[a, b, c]` — how a bracket literal is typed (fixed
  ...
  [type system](types.md#bracket-literals).
...
[definite assignment](types.md#definite-assignment)).
...
[exhaustiveness](types.md#match-exhaustiveness)).
...
a method of type `T` (see [methods](types.md#methods)); the standard library
...
type Name = | V1 { members } | V2       // sum type (leading | optional)
type Name: IfaceA, IfaceB = { members } // with interface constraints
```

A member is either a **field** (`name` or `name: Type` — the annotation is
optional) or a **method signature** (`name(params) -> Ret` with no body). A
method body inside a `type` block is a parse error: implementations go outside
the type, as `fun T.m(...)`, in the same module. Members are separated by
commas or newlines. Variants of a sum type are `Name` or `Name { members }`,
separated by `|`, which may start a new line.
...
[Modules](modules.md) for path resolution and visibility.
...
| `typeof(expr)` | the static type of `expr` (see [Reflection](reflection.md)) |
