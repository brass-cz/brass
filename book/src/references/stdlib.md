...
| `error(x)` | constructs `Result.Err` | see [Result](types.md#result) |
| `fields(x)`, `typeof(x)` | compile-time | see [Reflection](reflection.md) |
| `spawn(f)`, `with(c, f)`, `sync()` | concurrency | see [Concurrency](concurrency.md) |
...
| `parts.join(sep)` | `string[].(string) -> string` | join a *string array* with `sep` |
...
[type system](types.md#explicit-conversions).
...
type JsonValue =
    ...
    | Object { value }              // a HashMap from field name to JsonValue
```
...
| `j.into()` | `-> infer!` | decode into the type the call site expects — see [Reflection](reflection.md#generic-decoders-with---infer) |
