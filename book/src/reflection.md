...
prepoly can walk a record's fields at *compile time*: no runtime type
...
These pieces combine into reflective *deserialization* — filling a struct from
name-keyed data, or a whole JSON-to-struct decoder written once as
`fun Json.into(self) -> infer!`. See the
[reflection reference](references/reflection.md) for the complete rules and
the decoder pattern.
